...
declare function join <D extends string>(delimiter: D): <T extends string[]>(...parts: T) => JoinTuple<T, D>;

type JoinTuple<S extends unknown[], D extends string> = S extends [infer F, ...infer R] ? 
        R extends [] ?
            F 
            : F extends string ? 
                `${F}${D}${JoinTuple<R, D>}` 
